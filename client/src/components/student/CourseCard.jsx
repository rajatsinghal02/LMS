  ...
  return (
    <Link to={'/course/' + course._id} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg' onClick={()=> scrollTo(0,0)}>
      ...
      <div className='p-3 text-left'>
        ...
        <p className='text-gray-500'>Engineering Yuga</p>
        <div className='flex items-center space-x-2'>
          <p>{calculateRating(course)}</p>
          <div className='flex'>
            {[...Array(5)].map((_, i)=>(
              <img key={i} src= {i < Math.floor(calculateRating(course)) ? assets.star : assets.star_blank} alt="rating" className='w-3.5 h-3.5'/>
            ))}
          </div>
