...
const MyEnrollments = () => {
   ...
   const {enrolledCourses, calculateCourseDuration, navigate} = useContext(AppContext)
        ...
        const [progressArray, setProgressArray] = useState([
          ...
          {lectureCompleted: 6, totalLectures: 23},
          {lectureCompleted: 4, totalLectures: 34},
        ])
      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-10'>
        <thead className='text-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden'>
          <tr>
            ...
            <th className='px-4 py-3 font-semibold truncate'>Course Duration</th>
          ...
          <Line strokeWidth={2} percent={progressArray[index] ? (progressArray[index].lectureCompleted * 100 / progressArray[index].totalLectures) : 0 }  className='bg-gray-300 rounded-full'/>
            ...
            {progressArray[index] && `${progressArray[index].lectureCompleted} / ${progressArray[index].totalLectures} `} <span>lectures</span>
             ...
             <td className='px-4 py-3 max-sm:text-right'>
              ...
              <button className={`px-3 sm:px-5 py-1.5 sm:py-2 max-sm:text-xs text-white ${progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'bg-green-600' : 'bg-blue-600'}
